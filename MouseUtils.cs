...
internal class MouseUtils
{
    ...
    private static void MoveCursor(Point targetPoint, bool smoothly)
    {
        if (smoothly)
        {
            ...
            int steps = (int)(Math.Abs(targetPoint.X - cursorPosition.X) + 
                Math.Abs(targetPoint.Y - cursorPosition.Y) / 25000);
            ...
            for (int i = 0; i <= steps; i++)
            {
                ...
                //if(delay > 1 && t < .1f) delay --;
                //if(delay < 10 && t > .90f) delay ++;
    ...
    public static void SmoothMoveTo(Point pt)
    {
        MoveCursor(pt, true);
    }
